 ...
 * module).
 * 
 * <p>There is no assumed order or format for the CSV file.  Instead, users should implement a subclass of {@link MySQLFeedEventTranslator}
 ...
 * or an instantiated object can be constructed and passed to the {@link MySQLFeedCredentials} constructor.</p>
 * 
 * <p>To use the <code>MySQLFeed</code>, create a {@link MarketDataRequest} and pass it to the feed similarly to how other feeds process
 * market data requests.  This can be via the Strategy API, in which case the request should use 
 * {@link org.marketcetera.marketdata.MarketDataRequestBuilder#withProvider(String)} and specify {@link MySQLFeedModuleFactory#getProviderDescription()}
 * or by using the module framework and passing a <code>MarketDataRequest</code> to {@link MySQLFeedModule#requestData(org.marketcetera.module.DataRequest, org.marketcetera.module.DataEmitterSupport)}.</p>
 * 
